...
if project_env.exists() and project_env.stat().st_size > 0:
    load_dotenv(project_env)
if local_env.exists() and local_env.stat().st_size > 0:
...
def is_keys_configured() -> bool:
    ...
    return (
        api_key 
        and api_secret 
        and "testnet_api_key" not in api_key.lower()
        and "testnet_api_secret" not in api_secret.lower()
    )
...
def config():
    return jsonify({
        "keys_configured": is_keys_configured(),
        "testnet": BASE_URL,
    })
...
<div style="max-width:1100px;margin:0 auto;padding:22px 18px">
  ...
  <!-- CONFIG WARNING -->
  <div id="config-warning" style="background:rgba(255,159,64,0.15);border:1px solid rgba(255,159,64,0.4);border-radius:10px;padding:12px 16px;margin-bottom:16px;font-size:11px;color:#ffb347;display:none;line-height:1.6">
    <div style="font-weight:700;margin-bottom:4px">⚠ Binance API Keys Not Configured</div>
    <div>Edit <code style="background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:3px">r:\trading_bot\.env</code> and replace placeholder values with real testnet API keys.</div>
    <div style="margin-top:6px;color:#999">Order placement disabled until keys are configured.</div>
  </div>
...
async function submitOrder(){
  ...
  if(warning && warning.style.display!=='none'){showResult('✗ Configure API keys first',false);return;}
